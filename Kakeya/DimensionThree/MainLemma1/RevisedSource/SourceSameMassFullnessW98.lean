...
/-- The actual same-mass construction also retains the B3 fullness witness
on the SAME second shading, with the FULL original middle carrier denominator. -/
theorem exists_actual_same_mass_selections_with_middle_fullness_w98
              ...
              ((loss : ENNReal) ^ (6 : Nat))⁻¹ *
                ...
                fullness' (U.cover.indexSet block.b.val)
                  (fun Q => (zeroExtend selections.secondFamily (U.cover.tube block.b.val)
                    selections.secondShading Q).toShadedBody) := by  set_option maxHeartbeats 2000000 in
    exact (by
      ...
      have hactual_second_selected :
            ...
            ∀ (amb H : Finset iota) (Z : iota -> ShadedTube rho E)
              ...
              ∃ (mid F2 P2 : Finset iota)
                ...
                IsOneScaleSelected
                  ...
                  amb mid (zeroExtend mid Tmid Z) J Ttheta parent F2 Y2 P2 Z2 label lamP lamF := by
        ...
        refine ⟨mid, G.innerSet, G.outerSet, Yi, Yo, label,
          ...
          hmidne, hmidH, hchildne, hpatchMass, hpatchWeight, ?_⟩
        refine
      ...
      obtain ⟨mid, F2, P2, Y2, Z2, middleLabel, coarseFullness, middleFullness,
        hmid, hmidH, hF2, hpatchMass, hpatchWeight, second⟩ :=
      ...
      refine ⟨hselection_mono hLactual hpaid selections, ?_⟩
      change (((Cselect * Real.toNNReal
          ((2 + Real.log (1 / (delta : Real)) / Real.log 2) ^ (19 : Nat)) : NNReal) : ENNReal) ^ (6 : Nat))⁻¹ *
          (fullness' A (fun i => (Y i).toShadedBody)) ^ (2 : Nat) <=
        fullness' J (fun Q => (zeroExtend F2 (U.cover.tube block.b.val) Y2 Q).toShadedBody)
      rw [← hlambda]
