...
/-!
# The two every-scale conditions and Sticky estimate interfaces
...
(Lemma 7.4 + Lemma 7.5) remains dimension-general. The external producer
`stickyFrostmanEstimate` requires `Module.finrank ℝ E = 3`.
...
The original GWZ statement is for `E = ℝ³`. This Prop is defined in any dimension so that
the conditional implication `StickyFrostmanEstimate ⇒ StickyKatzTaoEstimate`
(Lemma 7.4 + Lemma 7.5) remains dimension-general. Its external input is produced by
`stickyFrostmanEstimate` only when `Module.finrank ℝ E = 3`.
