...
public import MyLeanRepo.InductionOnScales.SlopeCells
...
/-!
# Generalized Separated Packets Estimate

Variant of `separated_packets_estimate` with configurable separation factor.

`|TQ_local| ≥ |fineTubes| * m_Q / (c+1)`

Use `c=7` for the standard factor 8, or `c=8` for factor 9 if tighter
geometric disjointness requires it.
-/
...
section SeparatedPacketsEstimateGeneral

/-- Two local tubes are "c-separated" if different slope, or same slope
and intercept indices differ by at least c+1. -/
def AreSeparatedN {k : ℕ} (c : ℕ) (U1 U2 : DyadicTube k) : Prop :=
  U1.a ≠ U2.a ∨ |U1.b - U2.b| ≥ (c + 1 : ℤ)

/-- Generalized separated packets estimate with configurable separation. -/
lemma separated_packets_estimate_general
    {n k : ℕ}
    (c : ℕ)
    ...
    (h_disjoint : ∀ U1 ∈ fineTubes, ∀ U2 ∈ fineTubes,
      U1 ≠ U2 → AreSeparatedN c U1 U2 → Disjoint (packet U1) (packet U2)) :
    (TQ_local.card : ℝ) ≥ (fineTubes.card : ℝ) * (m_Q : ℝ) / ((c : ℝ) + 1) := by
  ...
  let S_a (a : ℤ) : Finset ℤ :=
    Classical.choose (exists_c_separated_subset c (interceptsAt a))
  have hS_a_sub : ∀ a ∈ slopeCells, S_a a ⊆ interceptsAt a := by
    intro a ha
    exact (Classical.choose_spec (exists_c_separated_subset c (interceptsAt a))).1
  have hS_a_sep : ∀ a ∈ slopeCells, ∀ b1 ∈ S_a a, ∀ b2 ∈ S_a a,
      b1 ≠ b2 → |b1 - b2| > (c : ℤ) := by
    intro a ha
    exact (Classical.choose_spec (exists_c_separated_subset c (interceptsAt a))).2.1
  have hS_a_card : ∀ a ∈ slopeCells,
      ((interceptsAt a).card : ℝ) ≤ ((c : ℝ) + 1) * ((S_a a).card : ℝ) := by
    intro a ha
    exact (Classical.choose_spec (exists_c_separated_subset c (interceptsAt a))).2.2
  ...
  have h_selected_separated : ∀ U1 ∈ selected, ∀ U2 ∈ selected,
      U1 ≠ U2 → AreSeparatedN c U1 U2 := by
    ...
    · subst h
      ...
      have h_far : |b1 - b2| > (c : ℤ) := hS_a_sep a1 ha1 b1 hb1 b2 hb2 h_bne
      have h_far' : |b1 - b2| ≥ (c + 1 : ℤ) := by linarith
      exact Or.inr h_far'
    · exact Or.inl h
  have h_packets_disjoint : ∀ U1 ∈ selected, ∀ U2 ∈ selected,
    ...
    have h_sep : AreSeparatedN c U1 U2 := h_selected_separated U1 hU1 U2 hU2 hne
  ...
  have h_fine_card : (fineTubes.card : ℝ) = ∑ a ∈ slopeCells, ((cellTubes a).card : ℝ) := by
    ...
    rw [h1] <;> simp
  have h_selected_card : (fineTubes.card : ℝ) ≤ ((c : ℝ) + 1) * (selected.card : ℝ) := by
    ...
    _ = ∑ a ∈ slopeCells, ((interceptsAt a).card : ℝ) := by
        ...
        exact h_eq
    _ ≤ ∑ a ∈ slopeCells, ((c : ℝ) + 1) * ((S_a a).card : ℝ) := by
        exact Finset.sum_le_sum (fun a ha => hS_a_card a ha)
    _ = ((c : ℝ) + 1) * ∑ a ∈ slopeCells, ((S_a a).card : ℝ) := by
        rw [Finset.mul_sum]
    _ = ((c : ℝ) + 1) * (selected.card : ℝ) := by rw [←h_selected_card_eq]
  ...
  have h_final : (selected.card : ℝ) * (m_Q : ℝ) ≥
      (fineTubes.card : ℝ) * (m_Q : ℝ) / ((c : ℝ) + 1) := by
    have h2 : (selected.card : ℝ) ≥ (fineTubes.card : ℝ) / ((c : ℝ) + 1) := by
      have h_pos : (0 : ℝ) < ((c : ℝ) + 1) := by positivity
      have h3 : (fineTubes.card : ℝ) / ((c : ℝ) + 1) ≤ (selected.card : ℝ) := by
        have h_pos' : (0 : ℝ) < ((c : ℝ) + 1) := by positivity
        have h4 : (fineTubes.card : ℝ) ≤ ((c : ℝ) + 1) * (selected.card : ℝ) := h_selected_card
        have h5 : (fineTubes.card : ℝ) / ((c : ℝ) + 1) ≤ (selected.card : ℝ) := by
          calc (fineTubes.card : ℝ) / ((c : ℝ) + 1)
            ≤ (((c : ℝ) + 1) * (selected.card : ℝ)) / ((c : ℝ) + 1) := by gcongr
          _ = (selected.card : ℝ) := by
            have h6 : (((c : ℝ) + 1) * (selected.card : ℝ)) / ((c : ℝ) + 1) = (selected.card : ℝ) := by
              have h7 : ((c : ℝ) + 1) ≠ 0 := h_pos'.ne'
              exact mul_div_cancel_left₀ (↑selected.card) h7

            exact h6
        exact h5
      exact h3
    have h_pos2 : 0 ≤ (m_Q : ℝ) := by positivity
    calc (selected.card : ℝ) * (m_Q : ℝ)
      ≥ ((fineTubes.card : ℝ) / ((c : ℝ) + 1)) * (m_Q : ℝ) := by gcongr
    _ = (fineTubes.card : ℝ) * (m_Q : ℝ) / ((c : ℝ) + 1) := by ring
  exact le_trans h_final h_main

end SeparatedPacketsEstimateGeneral
