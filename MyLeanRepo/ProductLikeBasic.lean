...
/-
Basic infrastructure for the product-like incidence theorem.
-/
...
lemma dyadicCubesMeeting_finite {d : ℕ} {δ : ℝ} (hδ : 0 < δ)
  ...
  · rcases (Set.nonempty_iff_ne_empty.mpr hA_empty) with ⟨x₀, hx₀⟩
    ...
    have h_main : dyadicCubesMeeting δ A ⊆ image (fun k : Fin d → ℤ => dyadicCube δ k) K := by
      intro Q hQ
...
lemma productLikeUnitGrid_finite {δ : ℝ} (hδ : 0 < δ) :
    (productLikeUnitGrid δ).Finite := by
  let N : ℤ := ⌈1 / δ⌉
  have hN_nonneg : 0 ≤ N := by
    dsimp only [N]
    have h₁ : 0 ≤ (1 / δ : ℝ) := by positivity
    apply Int.ceil_nonneg; exact h₁
  let S : Set ℤ := Set.Icc 0 N
  have hS_finite : S.Finite := Set.finite_Icc _ _
  have h₁ : productLikeUnitGrid δ ⊆ (fun k : ℤ => δ * (k : ℝ)) '' S := by
    intro x hx
    have h₂ : x ∈ productLikeIntegerGrid δ := hx.1
    have h₃ : 0 ≤ x := hx.2.1
    have h₄ : x ≤ 1 := hx.2.2
    rcases h₂ with ⟨k, hk⟩
    have h₅ : x = δ * (k : ℝ) := hk
    have h₆ : 0 ≤ (k : ℝ) := by
      rw [h₅] at h₃; exact (mul_nonneg_iff_of_pos_left hδ).mp h₃
    have h₇ : (k : ℝ) ≤ 1 / δ := by
      rw [h₅] at h₄
      have h₈ : (δ * (k : ℝ)) / δ ≤ 1 / δ := by gcongr
      have h₉ : (δ * (k : ℝ)) / δ = (k : ℝ) := by field_simp [hδ.ne'] <;> ring
      rw [h₉] at h₈; exact h₈
    have h₈ : k ∈ S := by
      simp only [S, Set.mem_Icc]; constructor
      · exact_mod_cast h₆
      · have h₉ : (k : ℝ) ≤ (N : ℝ) := by
          dsimp only [N]; exact h₇.trans (Int.le_ceil (1 / δ))
        exact_mod_cast h₉
    exact ⟨k, h₈, h₅.symm⟩
  exact Set.Finite.subset (hS_finite.image _) h₁

lemma productLikeUnitGrid_bounded {δ : ℝ} :
    Bornology.IsBounded (productLikeUnitGrid δ) := by
  have h₁ : productLikeUnitGrid δ ⊆ Set.Icc (0 : ℝ) 1 := by
    intro x hx; exact hx.2
  have h₂ : Bornology.IsBounded (Set.Icc (0 : ℝ) 1) := Metric.isBounded_Icc 0 1
  exact Bornology.IsBounded.subset h₂ h₁

end Grid



section AdditionalInfrastructure

/-- Combined positivity and upper bound for dyadic scales. -/
lemma dyadicScales_pos_and_le_one {δ : ℝ} (h : δ ∈ dyadicScales) :
    0 < δ ∧ δ ≤ 1 :=
  ⟨dyadicScales_pos h, dyadicScales_le_one h⟩

/-- The real-line copy of a bounded set is bounded. -/
...
/-- The real-line copy of a nonempty set is nonempty. -/
lemma productLikeRealLineCopy_nonempty {A : Set ℝ} (hA : A.Nonempty) :
  ...
  have h₃ : x 0 ∈ A := by
    rw [h₂]
    exact ha
  simpa [productLikeRealLineCopy, realLineCopy] using ⟨x, h₃⟩

/-- Increasing the constant C preserves the `(δ,s,C)`-set property. -/
