...
/-!
# Balanced dominating upper dilated covers

Start with a balanced strict cover by radius-`sigma` parents.  Enlarge those
parents coaxially to radius `rho`, choose a maximal essentially-distinct
subfamily, and reassign every complete old parent fiber to a retained
representative.

The fine family is not selected again.  Non-distinct upper parents give
support-free containment in a fixed `2000`-dilation.  If the upper conflict
degree is at most `D`, each retained representative absorbs at most `D + 1`
old fibers, so the new cover remains uniform with factor
`(D + 1) * C`.
-/
...
/--
A balanced strict lower cover produces a balanced essentially-distinct
upper dilated cover without changing the fine family.
-/
structure BalancedDominatingUpperDilatedCoverPackage
    {delta sigma rho : ℝ}
    {fine : TubeFamily delta} {lower : TubeFamily sigma}
    (P : TubeCover fine lower)
    (C : ENNReal) (D : ℕ) where
  upper : TubeFamily rho
  cover : DilatedTubeCover 2000 fine upper
  upperEssentiallyDistinct : upper.IsEssentiallyDistinct
  assignedUniform :
    cover.toFactoring.FibersAreCUniform (((D + 1 : ℕ) : ENNReal) * C)
  factoringFiberCount_lower :
    ∀ upperIndex, ∃ lowerIndex,
      P.toFactoring.fiberCount lowerIndex ≤
        cover.toFactoring.fiberCount upperIndex

def BalancedDominatingUpperDilatedCoverStatement : Prop :=
  ∀ {delta sigma rho : ℝ},
    ∀ {fine : TubeFamily delta} {lower : TubeFamily sigma},
    ∀ P : TubeCover fine lower,
    ∀ C : ENNReal,
    ∀ D : ℕ,
      ...
      Nonempty
        (BalancedDominatingUpperDilatedCoverPackage
          (rho := rho) P C D)

theorem balanced_dominating_upper_dilated_cover :
    BalancedDominatingUpperDilatedCoverStatement := by
  intro delta sigma rho fine lower P C D hfine hrho hsigma_rho
    hrho_one huniform hdegree
  classical
  ...
  have hconflict_containment :
    ...
    have hfirstVolume :
      ...
      Kakeya.Streamlined.RandomTranslation.tube_volume_eq_deltaTubeVolume
        (candidates.tube first)
    have hsecondVolume :
  ...
  have hnested :
    ...
    have hfine_lower :
        (fine.tube index).carrier ⊆
          (lower.tube (P.parent index)).carrier :=
      P.nested index
    have hlower_candidate :
        (lower.tube (P.parent index)).carrier ⊆
          (candidates.tube (P.parent index)).carrier := by
      simpa [candidates, sameAxisEnlargedFamily_tube] using
        carrier_subset_withRadius hsigma_rho
    ...
    exact hfine_lower.trans (hlower_candidate.trans hcandidates)
  ...
  let Q : DilatedTubeCover 2000 fine upper.family :=
    { parent := parentMap
