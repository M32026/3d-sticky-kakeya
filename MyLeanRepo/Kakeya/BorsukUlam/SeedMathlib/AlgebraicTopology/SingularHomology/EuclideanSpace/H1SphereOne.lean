...
public import MyLeanRepo.Kakeya.BorsukUlam.SeedMathlib.AlgebraicTopology.SingularHomology.EuclideanSpace.SphereZeroZerothHomotopy
...
public import MyLeanRepo.Kakeya.BorsukUlam.SeedMathlib.AlgebraicTopology.SingularHomology.H0MapIso
public import MyLeanRepo.Kakeya.BorsukUlam.SeedMathlib.AlgebraicTopology.SingularHomology.HomologyZeroPathComponents
public import MyLeanRepo.Kakeya.BorsukUlam.SeedMathlib.AlgebraicTopology.SingularHomology.MayerVietoris.HomologyZero
public import MyLeanRepo.Kakeya.BorsukUlam.SeedMathlib.AlgebraicTopology.SingularHomology.ReducedHomologyKernelIso
public import MyLeanRepo.Kakeya.BorsukUlam.SeedMathlib.CategoryTheory.Limits.KernelCompMono
public import MyLeanRepo.Kakeya.BorsukUlam.SeedMathlib.CategoryTheory.Limits.KernelIsoOfCompatibleIso
public import MyLeanRepo.Kakeya.BorsukUlam.SeedMathlib.CategoryTheory.Limits.SigmaBoolIsoBiprod
...
namespace SeedMathlib.AlgebraicTopology.SingularHomology.EuclideanSpace
...
noncomputable def h1SphereOneIsoInt :
  ...
  have h_f0π1 : f0 ≫ π1 = ε_UV ≫ inv ε_U := by
    ...
    calc
      f0 ≫ π1
        = (f0 ≫ π1) ≫ 𝟙 H0_U := by exact (Category.comp_id _).symm
      _ = (f0 ≫ π1) ≫ (ε_U ≫ inv ε_U) := by rw [IsIso.hom_inv_id ε_U]
      _ = ((f0 ≫ π1) ≫ ε_U) ≫ inv ε_U := by simp [Category.assoc]
      _ = ε_UV ≫ inv ε_U := by
        have h_goal : ((f0 ≫ π1) ≫ ε_U) ≫ inv ε_U = ε_UV ≫ inv ε_U := by
          exact h1 ▸ rfl
        exact h_goal
  have h_f0π2 : f0 ≫ π2 = (-ε_UV) ≫ inv ε_V := by
    ...
    calc
      f0 ≫ π2
        = (f0 ≫ π2) ≫ 𝟙 H0_V := by exact (Category.comp_id _).symm
      _ = (f0 ≫ π2) ≫ (ε_V ≫ inv ε_V) := by rw [IsIso.hom_inv_id ε_V]
      _ = ((f0 ≫ π2) ≫ ε_V) ≫ inv ε_V := by simp [Category.assoc]
      _ = (-ε_UV) ≫ inv ε_V := by
        have h_goal : ((f0 ≫ π2) ≫ ε_V) ≫ inv ε_V = (-ε_UV) ≫ inv ε_V := by
          exact h1 ▸ rfl
        exact h_goal
  ...
  let e_z0_bool_S0 : ZerothHomotopy S0 ≃ Bool := SeedMathlib.AlgebraicTopology.SingularHomology.EuclideanSpace.sphereZeroZerothHomotopyEquivBool
  ...
  let e_sigma_biprod : (∐ F_bool) ≅ R ⊞ R := SeedMathlib.CategoryTheory.Limits.sigmaBoolIsoBiprod R
  ...
  have h_desc_compat1 : g'_UV = e_coprod_uv.hom ≫ g_bool := by
    ...
    have h2 : Sigma.ι (fun (_ : SSet.π₀ X_UV) ↦ R) i ≫ (e_coprod_uv.hom ≫ g_bool) = 𝟙 R := by
      ...
      calc
        ...
        _ = 𝟙 R := by
          simpa [g_bool] using Sigma.ι_desc (fun (_ : Bool) ↦ 𝟙 R) (e_pi0_bool_uv i)
    rw [h, h2]
  ...
  have h_desc_compat2 : g_bool = e_sigma_biprod.hom ≫ biprod.desc (𝟙 R) (𝟙 R) := by
    ...
    · -- case true
      have h1 : Sigma.ι F_bool true ≫ g_bool = 𝟙 R := by
        simpa [g_bool] using Sigma.ι_desc (fun (_ : Bool) ↦ 𝟙 R) true
        ...
        calc
          ...
          _ = biprod.inl ≫ biprod.desc (𝟙 R) (𝟙 R) := by
            have h : Sigma.ι F_bool true ≫ e_sigma_biprod.hom = biprod.inl :=
              SeedMathlib.CategoryTheory.Limits.sigmaBoolIsoBiprod_ι_true R
            rw [h]
          _ = 𝟙 R := biprod.inl_desc _ _
      rw [h1, h2]
    · -- case false
      have h1 : Sigma.ι F_bool false ≫ g_bool = 𝟙 R := by
        simpa [g_bool] using Sigma.ι_desc (fun (_ : Bool) ↦ 𝟙 R) false
        ...
        calc
          ...
          _ = biprod.inr ≫ biprod.desc (𝟙 R) (𝟙 R) := by
            have h : Sigma.ι F_bool false ≫ e_sigma_biprod.hom = biprod.inr :=
              SeedMathlib.CategoryTheory.Limits.sigmaBoolIsoBiprod_ι_false R
            rw [h]
...
end SeedMathlib.AlgebraicTopology.SingularHomology.EuclideanSpace
