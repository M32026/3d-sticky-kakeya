import MyLeanRepo.Kakeya.CV.Isoperimetric.StructureTheorem.ReducedBoundaryOrientation
import MyLeanRepo.Kakeya.CV.Isoperimetric.StructureTheorem.TrueReducedBoundary
import MyLeanRepo.Kakeya.CV.Isoperimetric.StructureTheorem.VectorMeasureInner
import MyLeanRepo.Kakeya.CV.Isoperimetric.StructureTheorem.BlowUpScaling
import MyLeanRepo.Kakeya.CV.Isoperimetric.InnerMinkowski.Basic
import Mathlib.Tactic

/-!
# Weakened Orientation Theorem

Replaces the false `LebesgueDiffHyp` with a concrete differentiation hypothesis
for `|inner(ν(x), ν(y)) - 1|`, which follows from normal approximate continuity.

This module provides `OrientationHypothesesWeak` and `reduced_boundary_orientation_weak`,
which have the same conclusion as `reduced_boundary_orientation` but without the
overly strong `LebesgueDiffHyp` requirement.
-/
...
/-- Weakened orientation hypotheses: replaces `LebesgueDiffHyp` with concrete
differentiation of `|inner(ν, ν(y)) - 1|`. -/
structure OrientationHypothesesWeak (n : ℕ) (U : Set (E n)) (x : E n) (ν : E n) : Prop where
  hU : IsOpen U
  hn : 2 ≤ n
  h_perim_finite : perimeter U < ⊤
  hx : x ∈ trueReducedBoundary U
  hν : measureTheoreticNormal U x = ν
  h_leb_abs : Filter.Tendsto (fun r : ℝ =>
...
private lemma weak_orientation_error_bound
...
/-- Weakened orientation theorem: same conclusion as `reduced_boundary_orientation`
but without the false `LebesgueDiffHyp`. -/
theorem reduced_boundary_orientation_weak
    {n : ℕ} {U : Set (E n)} {x ν : E n}
    (h : OrientationHypothesesWeak n U x ν)
  ...
  rcases h with ⟨hU, hn, h_perim_finite, hx, hν, h_leb_abs, h_density_lower, h_density_upper⟩
  ...
  have h_gauss_green : ∀ (ψ : E n → ℝ), ContDiff ℝ ∞ ψ → HasCompactSupport ψ →
    ...
    have hvec_int : Integrable vec μ := by
      ...
      have h_norm_bound2 : ∀ᵐ y ∂(μ.restrict Set.univ), ‖vec y‖ ≤ C := by
        simpa [ae_restrict_mem] using (by filter_upwards with x using h_bdd_all x)
      have h_on2 : IntegrableOn vec Set.univ μ :=
  ...
  have h_err2 : |∫ y, ψ y * (h y - 1) ∂μ| < cφ * (μ (closedBall x (r * δ))).toReal := by
    have hMφ_pos : 0 < Mφ := by
      ...
      linarith
    apply weak_orientation_error_bound
