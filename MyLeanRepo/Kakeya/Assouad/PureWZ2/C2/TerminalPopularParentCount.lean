import MyLeanRepo.Kakeya.Assouad.PureWZ2.C2.TerminalPopularParents
import MyLeanRepo.Kakeya.Assouad.PureWZ2.C2.TerminalScaleParentCount

/-!
# Fixed-line counting for outer-popular terminal parents

The parent geometry selected from the outer-popular partial-cell carrier has
the same fixed-height, fixed-global-bin packing bound as the historical
whole-cell terminal parent package.  This file proves only that geometric
count.  It does not identify full balanced-parent volume with the volume of
the outer-popular carrier.
-/
...
/-- At most the standard fixed-line fiber bound many outer-popular slice cells
can be assigned to one genuine sticky parent. -/
theorem PureWZ2TerminalPopularFixedBinParentData.fiber_card
  ...
  have hfiberByY : fiber.card ≤ 2 * ys.card := by
    ...
    have hsubsetX :
        yFiber.image (fun cell => cell.1) ⊆
          Classical.choose
            (wz1Lemma23_global_bin_x_bound
              scale hscale popularPrepared.windowed.global.extendedSlope y
      ...
      have hrepHeight := line.representative_height cell hheavy
      have hzRaw := congrArg
        (fun index : ℤ × ℤ × ℤ => index.2.2) hrepIndex
      have hz :
          cell.2.2 = Int.floor (line.lineHeight / gridSide (scale / 2)) := by
        simpa [wz1Lemma23CellIndex, rhoGridIndex, gridIndex, hrepHeight] using
          hzRaw.symm
      have hbin :
          wz1Lemma23SnappedGlobalBin scale
              popularPrepared.windowed.global.extendedSlope cell =
            line.lineBin := by
        have hlabel :
            Int.floor
              (wz1Lemma23GlobalCoordinate
                  popularPrepared.windowed.global.extendedSlope
    ...
    have hinjective : Set.InjOn (fun cell : ℤ × ℤ × ℤ => cell.1) yFiber := by
      ...
      have hfirstIndex := line.representative_index first hfirstHeavy
      have hsecondIndex := line.representative_index second hsecondHeavy
      have hfirstHeight := line.representative_height first hfirstHeavy
      have hsecondHeight := line.representative_height second hsecondHeavy
      have hzFirstRaw := congrArg
        (fun index : ℤ × ℤ × ℤ => index.2.2) hfirstIndex
      have hzSecondRaw := congrArg
        (fun index : ℤ × ℤ × ℤ => index.2.2) hsecondIndex
      have hzFirst : first.2.2 =
          Int.floor (line.lineHeight / gridSide (scale / 2)) := by
        simpa [wz1Lemma23CellIndex, rhoGridIndex, gridIndex, hfirstHeight]
          using hzFirstRaw.symm
      have hzSecond : second.2.2 =
          Int.floor (line.lineHeight / gridSide (scale / 2)) := by
        simpa [wz1Lemma23CellIndex, rhoGridIndex, gridIndex, hsecondHeight]
          using hzSecondRaw.symm
      apply Prod.ext
      · exact heq
      · apply Prod.ext
        ...
        rw [show
          (((firstY : ℝ) + 1 / 2) * gridSide (scale / 2) -
            ...
            ((firstY - secondY : ℤ) : ℝ) * gridSide (scale / 2) by
              push_cast; ring]
        rw [abs_mul, hside,
          abs_of_pos (by positivity : 0 < scale / Real.sqrt 3)]
...
/-- The fixed-line heavy cells are controlled by the number of genuine sticky
parents selected from the outer-popular carrier. -/
theorem PureWZ2TerminalPopularFixedBinParentData.heavy_card
    {sigma inputLoss delta stickyLoss : ℝ}
    {logExponent : ℕ}
    {source : PureWZ2QuantitativeGrainConfiguration sigma inputLoss delta}
    {terminal : PureWZ2TerminalScaleStickyData source stickyLoss logExponent}
