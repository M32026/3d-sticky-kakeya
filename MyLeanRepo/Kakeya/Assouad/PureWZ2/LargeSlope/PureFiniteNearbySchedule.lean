import MyLeanRepo.Kakeya.Assouad.PureWZ2.LargeSlope.PureFiniteNearbyScheduleStatements
...
theorem pureWZ2_finite_pure_nearby_schedule :
    PureWZ2FiniteNearbyScheduleStatement := by
  classical
  ...
  set requested : Fin scaleCount → WZ2PaperRequestedScale delta := fun i =>
  ...
  set witness : ∀ i : Fin scaleCount,
      WZ2PaperPureNearbyScaleCoverData family (requested i) C :=
    fun i => Classical.choice (hcwa.2.2.2 (requested i))
    with hwitness_def
  ...
  let q (rho₀ : WZ2PaperRequestedScale delta) : ℕ → Prop :=
    fun n => rho₀.1 ≤ s n
  have hqk : ∀ rho₀, q rho₀ k := by
    ...
    rw [hs_k]
    exact h
  let j_func : WZ2PaperRequestedScale delta → ℕ := fun rho₀ =>
    Nat.find ⟨k, hqk rho₀⟩
  ...
  let representative : WZ2PaperRequestedScale delta → Fin scaleCount :=
