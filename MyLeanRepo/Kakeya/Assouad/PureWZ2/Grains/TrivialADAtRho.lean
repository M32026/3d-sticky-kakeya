import MyLeanRepo.Kakeya.Assouad.PureWZ2.Grains
import MyLeanRepo.Kakeya.Assouad.PureWZ2.Grains.TrivialCovering
import Mathlib.Tactic

/-!
# Trivial AD bound at a specific scale

Provides `trivial_ad_at_rho`: a variant of the diameter-based AD bound
that works when the scale `rho` is large enough to satisfy
`3 * (2/√rho)^σ ≤ δ^(-outputLoss)`.

Unlike `easy_regime_ad`, this does not require `outputLoss > σ/2`;
instead it takes a rho-specific hypothesis. This is useful for closing
parts of the hard regime where `rho` is sufficiently large.

## Main result

- `trivial_ad_at_rho`: `PureWZ2PaperADSet1 E ρ (1-σ) C` from a diameter bound
- `trivial_ad_key_at_rho`: the key real inequality
-/
...
/-- Key inequality for trivial AD bound at a specific rho.

Given `3 * (2 / Real.sqrt rho)^sigma ≤ delta ^ (-outputLoss)`,
proves `3 * x ≤ delta^(-outputLoss) * x^(1-sigma)` for all `1 ≤ x ≤ 2/√rho`. -/
lemma trivial_ad_key_at_rho
    ...
    (hrho_pos : 0 < rho)
    (h_main : 3 * (2 / Real.sqrt rho)^sigma ≤ delta ^ (-outputLoss))
    {x : ℝ} (hx1 : 1 ≤ x) (hx2 : x ≤ 2 / Real.sqrt rho) :
    3 * x ≤ (delta ^ (-outputLoss)) * (x ^ (1 - sigma)) := by
  have hx_pos : 0 < x := by linarith
  have h1 : x ^ sigma ≤ (2 / Real.sqrt rho) ^ sigma :=
    Real.rpow_le_rpow (by linarith) hx2 (by linarith)
  have h2 : 3 * x ^ sigma ≤ delta ^ (-outputLoss) := by
    calc
      3 * x ^ sigma ≤ 3 * (2 / Real.sqrt rho) ^ sigma := by gcongr
      _ ≤ delta ^ (-outputLoss) := h_main
  have h4 : x ^ sigma * x ^ (1 - sigma) = x := by
    have h5 : x ^ sigma * x ^ (1 - sigma) = x ^ (sigma + (1 - sigma)) :=
      Eq.symm (Real.rpow_add hx_pos sigma (1 - sigma))
    have h6 : sigma + (1 - sigma) = 1 := by ring
    rw [h5, h6]
    have h7 : x ^ (1 : ℝ) = x := by simp
    rw [h7]
  have h7 : 0 ≤ x ^ (1 - sigma) := by positivity
  have h8 : (3 * x ^ sigma) * x ^ (1 - sigma) ≤ (delta ^ (-outputLoss)) * x ^ (1 - sigma) := by
    gcongr
    <;> linarith
  have h9 : 3 * x = (3 * x ^ sigma) * x ^ (1 - sigma) := by
    have h10 : 3 * x = 3 * (x ^ sigma * x ^ (1 - sigma)) := by rw [h4] <;> ring
    rw [h10] <;> ring
  rw [h9]
  exact h8

/-- Trivial AD bound at a specific rho.

Given E contained in an interval of length `2√ρ` and the key inequality
`3 * (2/√ρ)^σ ≤ δ^(-outputLoss)`, proves `PureWZ2PaperADSet1 E ρ (1-σ) C`.

This is a rho-specific variant of `easy_regime_ad` that works in the hard
regime when `rho` is sufficiently large. -/
lemma trivial_ad_at_rho
    {delta sigma outputLoss rho : ℝ}
    {E : Set ℝ} {C : ENNReal}
  ...
  have h_key : ∀ (x : ℝ), 1 ≤ x → x ≤ 2 / Real.sqrt rho →
    ...
    fun x hx1 hx2 => trivial_ad_key_at_rho hdelta_pos hdelta_le_one hsigma_pos hsigma_lt_one
      houtputLoss_pos hrho_pos h_main hx1 hx2
