import MyLeanRepo.Kakeya.Cinematic.Section5.IncidenceCoarseFiberNonconcentrationInputs
import Mathlib.Algebra.BigOperators.Group.Finset.Basic
import Mathlib.Algebra.Order.BigOperators.Group.Finset

/-!
# Non-concentration from selected incidences

This is the direct bridge between the selected function--fine-rectangle
incidence graph and the abstract counting core of PYZ Lemma 47.
-/

namespace Kakeya.Cinematic

theorem incidence_coarse_fiber_nonconcentration :
    IncidenceCoarseFiberNonconcentrationStatement := by
  intro β γ _ _ _ hDoubleCount hNonconcentration edges parent level hReg coarse hRectNonempty
  intro mu₁ mu₂ coefficient logLoss hMu2Pos hCoeffNonneg hLogLossOne hMu1Le hAggregate hBallUpper
  ...
  let G : Fin M → FiniteFunctionFamily := incidenceActiveFiberFamily edges parent coarse
  ...
  have hCarrier : ∀ i : Fin M, (G i).carrier ⊆ H.carrier := by
    ...
    intro f hf
    have h_f_in_fiber : f ∈ incidenceRectangleFiber edges rect := by exact_mod_cast hf
  ...
  have hDegreeUpper : ∀ f ∈ funcSupport, incidenceDegree edges parent f coarse < 2 * q := by
    intro f hf
    have hPos : 0 < incidenceDegree edges parent f coarse := (h1 f).mp hf
    rcases hReg f coarse with hzero | hrange
    · rw [hzero] at hPos
      contradiction
    · have h : incidenceDegree edges parent f coarse < 2 ^ (level + 1) := hrange.2
  ...
  have hLowerAll : ∀ (center : C2Function) (radius : ℝ),
      (q : ℝ) * ((H.carrier ∩ c2Ball center radius).ncard : ℝ) ≤
        ∑ i : Fin M, (((G i).carrier ∩ c2Ball center radius).ncard : ℝ) := by
    intro center radius
    let ballSet : Set C2Function := c2Ball center radius
  ...
  have hUpperAll : ∀ (center : C2Function) (radius : ℝ),
      (∑ i : Fin M,
          (((G i).carrier ∩ c2Ball center radius).ncard : ℝ)) ≤
        (M : ℝ) * mu₁ := by
    intro center radius
    have h_each : ∀ i : Fin M,
        (((G i).carrier ∩ c2Ball center radius).ncard : ℝ) ≤ mu₁ := by
      intro i
      let rect := incidenceActiveRectangle edges parent coarse i
      have hRectMem : rect ∈ rectSupport :=
        ((Finset.equivFin rectSupport).symm i).property
      exact hBallUpper rect hRectMem center radius
    calc
      ∑ i : Fin M,
          (((G i).carrier ∩ c2Ball center radius).ncard : ℝ) ≤
          ∑ i : Fin M, mu₁ :=
  ...
  exact hNonconcentration
    (M := M) (q := q) (mu₁ := mu₁) (mu₂ := mu₂)
    (coefficient := coefficient) (logLoss := 2 * logLoss)
    hq_pos hMu2Pos hCoeffNonneg (by linarith) hMu1Le
    H G hCarrier hAggregateUpper hLowerAll hUpperAll
