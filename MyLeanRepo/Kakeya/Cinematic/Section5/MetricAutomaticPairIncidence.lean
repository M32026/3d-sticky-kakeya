import MyLeanRepo.Kakeya.Cinematic.Section5.MetricAutomaticPairIncidenceInputs
import MyLeanRepo.Kakeya.Cinematic.Section5.RepresentativeMetricAutomaticGoodPairs

/-!
# Count fine rectangles in the metric-automatic regime
-/

namespace Kakeya.Cinematic

theorem metric_automatic_pair_incidence :
    MetricAutomaticPairIncidenceStatement := by
  ...
  intro family E delta diameter epsilon eta tRep DeltaRep C_R
    metricLower tangencyCut Cc hFamily hdelta hC_R
    hmetricLower_pos hmetricLower_lt_one
    htangencyCut_pos htangencyCut_lt_one
    data hmetricAuto htangencyScale htangencySmall hsmall hCc hCc_delta
  intro R source hrect hcentral hincomp F_B hF_B h_fiber_subset hfiber_card
    q hq hfiber_lower
  classical
  let I := data.interval
  ...
  have hmetricLower_pos' : 0 < metricLower := hmetricLower_pos
  have htangencyLower_pos : 0 < tangencyLower := by positivity
  let ambientFinset := F_B.toFinset
  let pairPred : C2Function × C2Function → Prop := fun p =>
    metricLower < c2Distance p.2 p.1 ∧
      tangencyCut * DeltaRep / 2 ≤ tangencyParameterOn I p.2 p.1 + delta
  ...
  have hinc_sub : ∀ i, incidence i ⊆ pairs := by
    ...
    have h4 : p.1 ∈ ambientFinset := by
      ...
      have h6 : p.1 ∈ F_B.carrier := h_fiber_subset i h5
    ...
    have h7 : p.2 ∈ ambientFinset := by
      ...
      have h9 : p.2 ∈ F_B.carrier := h_fiber_subset i h8
  ...
  have h_lower : ∀ (i : Fin R.card),
    ...
    intro i _
    let G := data.assignment.fiber (source i)
    have hGood : G.card ^ 2 ≤ 3 * (incidence i).card :=
      representative_metric_automatic_good_pairs
        (family := family) (E := E) (K := K) (delta := delta)
        (diameter := diameter) (epsilon := epsilon) (eta := eta)
        (tRep := tRep) (DeltaRep := DeltaRep) (C_R := C_R)
        hdelta data (source i) metricLower tangencyCut
        hmetricAuto htangencyCut_pos htangencyCut_lt_one
        (htangencyScale (source i)) htangencySmall (hfiber_card i)
  ...
  have h_upper : ∀ (p : C2Function × C2Function), p ∈ pairs →
    ...
    have hmetric : metricLower ≤ c2Distance w b := by
      have h : metricLower < c2Distance p.2 p.1 := hpair.1
      simpa [w, b] using h.le
    have htangency :
        tangencyLower ≤ tangencyParameterOn I w b + delta := by
      have h : tangencyCut * DeltaRep / 2 ≤
          tangencyParameterOn I p.2 p.1 + delta := hpair.2
      simpa [w, b, tangencyLower] using h
  ...
  have hfinal :
    ...
    have h8 :
      ...
      nlinarith
    exact h6.trans h8
  simpa [x, t, tangencyLower] using hfinal
