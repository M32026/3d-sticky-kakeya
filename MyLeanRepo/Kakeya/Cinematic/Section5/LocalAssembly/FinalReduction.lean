...
import MyLeanRepo.Kakeya.Cinematic.Section5.LocalAssembly.LogAbsorption

/-!
# Final restricted weak-type reduction for the local cinematic estimate

The geometric part of Section 5 is isolated as a uniform dyadic level-set
measure estimate. This module proves that estimate implies the frozen local
integral output, including Katz--Tao cardinality control and absorption of the
dyadic logarithm.
-/
...
theorem localAssembly_integral_of_level_set
    (hMultiplicity : MultiplicityLevelSelectionStatement)
    (hRestricted : RestrictedWeakTypeReductionStatement)
    (hLevel : WZ2LocalLevelSetInput) :
    WZ2LocalIntegralInput := by
  intro K D C_KT lambda hK hD hC_KT hlambda
  ...
  rcases hLevel K D C_KT lambda hK hD hC_KT hlambda
      family hfamily (epsilon / 2) (by positivity) with
  ...
  intro delta hdelta hdelta₀ F hF_family hF_separated hF_KT c
  ...
  have hlevel_bound :
    ...
    exact hlevel delta hdelta hdelta_level F hF_family hF_separated
      hF_KT c mu hmu E₀ hE₀ hE₀_sub hmu_level
  have hintegral :=
